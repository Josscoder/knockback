...
func openCreatePresetForm(p *player.Player) {
	...
	custom := &form.Custom{
		...
		Elements: []form.Element{
			...
			form.Input{Text: "Attack cooldown (seconds)", Placeholder: "0.1", Default: formatFloat(cfg.AttackCooldown), Submit: func(v string) { cooldown = v }},
...
func buildConfigFromStrings(horizontal, vertical, cooldown, limiter string) (*config.KnockbackConfig, error) {
	...
	cooldownSec, err := parseNonNegativeFloat(cooldown, "attack cooldown")
	if err != nil {
		return nil, err
	}
...
func renderConfig(cfg *config.KnockbackConfig) string {
	...
	return fmt.Sprintf(
		"horizontal=%.3f vertical=%.3f cooldown=%.3fs height_limiter=%d",
		cfg.HorizontalForce,
		cfg.VerticalForce,
		cfg.AttackCooldown,
		cfg.HeightLimiter,
