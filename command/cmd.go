...
func openCreatePresetForm(p *player.Player, manager *knockback.Manager) {
	...
	custom := &form.Custom{
		...
		Elements: []form.Element{
			...
			form.Input{Text: "Attack cooldown", Placeholder: "0", Default: formatFloat(cfg.AttackCooldown), Submit: func(v string) { cooldown = v }},
...
func buildConfigFromStrings(horizontal, vertical, cooldown, limiter, factor string) (*config.KnockbackConfig, error) {
	...
	c, err := parseNonNegativeFloat(cooldown, "attack cooldown")
	if err != nil {
		return nil, err
	}
